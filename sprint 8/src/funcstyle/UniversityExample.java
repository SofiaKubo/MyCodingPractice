...
public class UniversityExample {
    ...
    public static void main(String[] args) {
        ...
        Set<String> examPassedNames = new HashSet<>(); examPassedNames.add("Иванов Иван");
        examPassedNames.add("Практикумова Яна");

        //соответствие года поступления и названия группы
        Map<Integer, String> groupNames = new HashMap<>(); groupNames.put(2020, "2020-ГР1");
        ...
        List<Student> graduatedStudents = students.stream()
                .filter(student -> examPassedNames.contains(student.surname + " " + student.name)) /* проверка, что студент успешно сдал экзамен */.map(student -> {
                    student.groupName = groupNames.get(student.entranceYear); // заполнение названия группы студента
                    return student;
                }) /* заполнение названия группы студента */
                // операция peek выполняет над элементом действие и передаёт далее тот же элемент
                .peek(student -> graduatesClub.add(student.email)) /* добавление студента в клуб выпускников */.collect(Collectors.toList());
...
class Student {
    ...
    public Student(String surname, String name, String email, int entranceYear) {
        this.surname = surname; this.name = name; this.email = email; this.entranceYear = entranceYear;
    }

    @Override
    public String toString() {
        return "Student{" + "surname='" + surname + '\'' + ", name='" + name + '\'' + ", email='" + email + '\'' + ", entranceYear=" + entranceYear + ", groupName='" + groupName + '\'' + '}';
    }
}
